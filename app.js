...
const salutTextContainer = document.querySelector(".textContainer");
const imgContainer = document.querySelector(".imgContainer");
const Devname = document.querySelector(".name"); 
const jobContainer = document.querySelectorAll(".jobContainer");
const jobT = document.querySelectorAll(".jobTitle");
...
window.addEventListener("scroll",()=>{
    ...
    salutTextContainer.style.transform = `translateY(${offsetY * 0.1}px)` ; 
    imgContainer.style.transform = `translate(${offsetY * 0.4}px , ${offsetY * 0.7}px)` ;
    Devname.style.transform = `translateX(${offsetY * 0.2}px)` ;
    jobContainer[0].style.backgroundPositionY = `${offsetY * 0.5}px`
    jobContainer[1].style.backgroundPositionY = `${-offsetY * 0.5}px`
    jobT[0].style.transform =  `translateX(calc(200vh - ${offsetY}px))`
    jobT[1].style.transform =  `translateX(calc(-300vh + ${offsetY}px))`
...
function showMessageForMobile() {
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        alert("Please open this website on a laptop or desktop computer.");
    }
}

window.onload = showMessageForMobile;
